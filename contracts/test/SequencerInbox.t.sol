...
contract SequencerInboxTest is SequencerBaseSetup {
    ...
    function test_force_inclusion() external {
        ...
        bytes memory rlpEncodedTransaction = bytes("");

        seqIn.sendRLPEncodedTx(rlpEncodedTransaction);

        assertEq(seqIn.delayedMessageCounter(), 1);

        vm.warp(block.timestamp + 86401);
        vm.roll(block.number + 5761);

        uint256 lastL2Block = 0;
        uint256 lastL2Timestamp = 0;
        bytes memory txContextHash = abi.encodePacked(
            sequencerAddress,
            lastL2Block,
            lastL2Timestamp
        );
        ...
        bytes32 prevAcc = 0;
        uint256 prevNumTxs = 0;
        bytes32 prevTxDataHash = 0;
        ...
        seqIn.forceInclusion(
            ...
            0,
            l1BlockAndTime,
            keccak256(rlpEncodedTransaction)
        );
 ...
 /////////////////////////////////
    ...
    //     // Each context corresponds to a single "L2 block"
    //     uint256 numTxns = numTxnsPerBlock * txnBlocks;
    //     console.log("num of Txs = ", numTxns);
    //     uint256 numContextsArrEntries = 3 * txnBlocks; // Since each `context` is represented with uint256 triplet: (numTxs, l2BlockNumber, l2Timestamp)
    ...
    //     // console.log("encoded data HashCalc");
    //     // console.logBytes(dataHashCalc);
    ...
    //         txnL2BlockNumber = contexts[numContextsArrEntries-2];
    //         txnL2Timestamp = contexts[numContextsArrEntries-1];
    //         console.log("txnL2BlockNumber: ",txnL2BlockNumber);
    //         console.log("txnL2Timestamp: ",txnL2Timestamp);
    ...
    //         batchNum = 0; //seqIn.accumulators(0) returns a non-zero value while seqIn.accumulators(1) reverts.
    //         numTxsBefore = numTxns; // since we are verifying the inclusion of the 1st transaction of the first block.
    ...
    //         accBefore = seqIn.testRunAcc(); // again, since this is our first inclusion, acc before that would have been empty bytes too.
    ...
    //             address(bob),
    //             seqIn.l1BlockAndTime(0),
    //             seqIn.l1BlockAndTime(1),
    //             uint256(0),
    //             seqIn.baseFee(),
    //             keccak256(dataHashCalc)
    //         );
    //         console.log("encodedTx");
    //         console.logBytes(encodedTx);

    //         // bytes memory encodedTx = "";


    //         // console.logBytes32(keccak256(bytes()));
    //         console.logBytes(proof);
    //         vm.prank(alice); // Alice wants to verify if the 1st transaction is indeed included or not
    //         seqIn.verifyDelayedTxInclusion(
    //             encodedTx,
    //             proof
    //         );
