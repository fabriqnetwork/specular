...
import {RLPReader2} from "../rlp/RLPReader2.sol";
...
library MerkleTrie {
        ...
        for (uint256 i = 0; i < _proof.length; i++) {
            ...
            if (currentNode.decoded.length == BRANCH_NODE_LENGTH) {
                if (currentKeyIndex == key.length) {
                    ...
                    RLPReader2.RLPItem memory nextNode = currentNode.decoded[branchKey];
    ...
    function _parseProof(bytes[] memory _proof) private pure returns (TrieNode[] memory) {
        ...
        for (uint256 i = 0; i < length;) {
            proof[i] = TrieNode({encoded: _proof[i], decoded: RLPReader2.readList(_proof[i])});
            unchecked {
                ++i;
            }
        }
     ...
     */
    function _getNodeID(RLPReader2.RLPItem memory _node) private pure returns (bytes memory) {
        return _node.length < 32 ? RLPReader2.readRawBytes(_node) : RLPReader2.readBytes(_node);
    }
    ...
    function _getNodePath(TrieNode memory _node) private pure returns (bytes memory) {
        return Bytes.toNibbles(RLPReader2.readBytes(_node.decoded[0]));
    }
    ...
    function _getNodeValue(TrieNode memory _node) private pure returns (bytes memory) {
        return RLPReader2.readBytes(_node.decoded[_node.decoded.length - 1]);
    }
