...
function useFinalizeDeposit() {
  ...
  const finalizeDeposit = async (wallet: wallet, amount: ethers.BigNumberish, pendingDeposit:PendingDeposit): Promise<void> => {
    ...
    await wallet.provider.switchNetworkAsync(SPECULAR_NETWORK_ID);
    ...
    await wallet.provider.switchNetworkAsync(CHIADO_NETWORK_ID);
  };
  ...
  return { finalizeDeposit, data, resetData };
}
