...
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if ctx.IsSet(specularUtils.RollupNodeFlag.Name) {
		...
		vmConfig.SpecularEVMPreTransferHook = hook.MakeSpecularEVMPreTransferHook(rollupConfig)
		vmConfig.SpecularL1FeeReader = hook.MakeSpecularL1FeeReader(rollupConfig)
