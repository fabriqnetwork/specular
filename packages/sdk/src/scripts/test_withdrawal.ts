...
async function main() {
    ...
    const withdrawalETHResponse = await crossChainMessenger.withdrawETH(200);

    // // 2 block confirmations
    const depositETHReceipt = await withdrawalETHResponse.wait(2);

    const finalizeWithdrawalResponse = await crossChainMessenger.finalizeWithdrawal(depositETHReceipt);
