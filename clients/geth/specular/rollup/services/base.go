...
func NewMockBaseService(eth *rollup_mock.MockBackend, proofBackend proof.Backend, cfg *Config, auth *bind.TransactOpts) (*MockBaseService, error) {
	// if eth == nil {
	// 	return nil, fmt.Errorf("can not use light client with rollup")
	// }
	ctx, cancel := context.WithCancel(context.Background())
	l1, err := ethclient.DialContext(ctx, cfg.L1Endpoint)
	if err != nil {
		log.Info("[BASE] ----> NewBaseService, this is err in ethclient DialContext", "err", err)
		cancel()
		return nil, err
	}
	callOpts := bind.CallOpts{
	...
	log.Info("[BASE] ----> NewBaseService this is cfg.SequencerInboxAddr: ", "cfg.SequencerInboxAddr", cfg.SequencerInboxAddr)
	inbox, err := bindings.NewISequencerInbox(common.Address(cfg.SequencerInboxAddr), l1)
	if err != nil {
		log.Info("[BASE] ----> NewBaseService, this is err in binds.NewISequencerInbox", "err", err)
		cancel()
		return nil, err
	}
	inboxSession := &bindings.ISequencerInboxSession{
	...
	rollup, err := bindings.NewIRollup(common.Address(cfg.RollupAddr), l1)
	if err != nil {
		log.Info("[BASE] ----> NewBaseService, this is err in binds.NewIRollup", "err", err)
		cancel()
		return nil, err
	}
	rollupSession := &bindings.IRollupSession{
	...
	assertionMapAddr, err := rollupSession.Assertions()
	if err != nil {
		log.Info("[BASE] ----> NewBaseService, this is err in rollupSession.Assertions", "err", err)
		cancel()
		return nil, err
	}
	assertionMap, err := bindings.NewAssertionMapCaller(assertionMapAddr, l1)
	if err != nil {
		log.Info("[BASE] ----> NewBaseService, this is err in assertionMap", "err", err)
		cancel()
		return nil, err
	}
	assertionMapSession := &bindings.AssertionMapCallerSession{
	...
	log.Info("[BASE] ----> this is eth: ", "eth", eth)
	//log.Info("[BASE] ----> this is eth.Blockchain: ", "eth.Blockchain", eth.Blockchain())
	b.Chain = eth.BlockChain()
	return b, nil
}
