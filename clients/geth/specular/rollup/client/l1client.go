...
type L1BridgeClient interface {
	...
	Close()
	// ISequencerInbox.sol
	AppendTxBatch(contexts []*big.Int, txLengths []*big.Int, firstL2BlockNumber *big.Int, txBatchVersion *big.Int, txBatch []byte) (*types.Transaction, error)
...
func (c *EthBridgeClient) AppendTxBatch(contexts []*big.Int, txLengths []*big.Int, firstL2BlockNumber *big.Int, txBatchVersion *big.Int, txBatch []byte) (*types.Transaction, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	f := func() (*types.Transaction, error) {
		return c.inbox.AppendTxBatch(contexts, txLengths, firstL2BlockNumber, txBatchVersion, txBatch)
	}
